...
Для построения графиков экспериментов дополнительно можно установить
Python-пакеты `pandas` и `matplotlib`. Без них сценарий выводит таблицу
агрегированных результатов и пропускает генерацию изображений.
...
Скрипт создаёт входные данные, выполняет по 10 запусков сжатия и распаковки для
каждого файла и выводит агрегированные результаты. Если установлены
`pandas` и `matplotlib`, он также строит графики. Пакеты для графиков можно
...
- `main.c` отвечает за разбор аргументов командной строки и выбор режима работы.
- `huffman_codec.c` реализует сжатие, распаковку и работу с заголовком архива.
- `huffman_tree.c` строит и освобождает дерево Хаффмана по таблице частот.
- `bitio.c` реализует побитовый ввод-вывод для записи и чтения сжатого потока.
